...
public class Paths {
    ...
    public static <T> int[] pathLengthAndGold(List<Node> path) {
    	synchronized(path) {
    		if(path.size() == 0) return new int[]{0,0};
    		
    		Iterator<Node> iter = path.iterator();
    		Node p = iter.next();
    		//First Node on path
    		int[] s = {0,0};
    		s[1] = p.getTile().getGold();
    		//invariant: s is the sum of edges from start up to Node p
    		while(iter.hasNext()) {
    			Node q = iter.next();
    			s[0] = s[0] + p.getEdge(q).length;
    			s[1] = s[1] + q.getTile().getGold();
    			p = q;
    		}
    		return s;
    	}
